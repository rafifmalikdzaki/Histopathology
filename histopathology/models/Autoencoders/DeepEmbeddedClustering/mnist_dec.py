...
class CachedMNIST(Dataset):
    def __init__(self, train, cuda, testing_mode=False):
        ...
        self.ds = MNIST("./data", download=True,
                        train=train, transform=img_transform)
        self.cuda = cuda
    ...
    def _transformation(img):
        return (
            torch.ByteTensor(
                torch.ByteStorage.from_buffer(img.tobytes())).float()
            * 0.02
        )
...
def main(cuda, batch_size, pretrain_epochs, finetune_epochs, testing_mode):
    ...
    print("DEC stage.")
    model = DEC(cluster_number=10, hidden_dimension=10,
