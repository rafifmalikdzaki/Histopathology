...
from .histopathology_dataset import ImageDataset, create_dataset
import wandb
...
class RobustDAE(pl.LightningModule):
    def __init__(
        ...
        self.val_lpips = torchmetrics.image.lpip.LearnedPerceptualImagePatchSimilarity(net_type='vgg')
    ...
    def _log_reconstructions(self, original, reconstructed):
        ...
        for i in range(min(4, len(original))):
            ...
            sample_psnr = torchmetrics.functional.psnr(
                reconstructed[i].unsqueeze(0), 
                original[i].unsqueeze(0)
            ).item()
            
            sample_ssim = torchmetrics.functional.structural_similarity_index_measure(
                reconstructed[i].unsqueeze(0), 
                original[i].unsqueeze(0)
            ).item()
...
def train_robust(config_path=None) -> None:
    ...
    # Create datasets without device transfer (pin_memory will handle GPU transfer)
    train_ds = ImageDataset(X_train, y_train, device=None)
    val_ds = ImageDataset(X_val, y_val, device=None)
    ...
    callbacks = [
        ...
        ModelCheckpoint(
            ...
            save_top_k=callbacks_config.get('model_checkpoint', {}).get('save_top_k', 3),
...
def create_wandb_config(path='configs/wandb_config.yaml'):
