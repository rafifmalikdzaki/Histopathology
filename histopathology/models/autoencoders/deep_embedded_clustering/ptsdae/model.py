        ...
        if update_freq is not None and epoch % update_freq == 0:
            if validation_loader is not None:
                ...
                validation_loss = loss_function(
                    validation_output, validation_actual)
...
def pretrain(
    ...
    for index in range(number_of_subautoencoders):
        ...
        sub_autoencoder = DenoisingAutoencoder(
            ...
            corruption=nn.Dropout(
                corruption) if corruption is not None else None,
        )
        if cuda:
            sub_autoencoder = sub_autoencoder.cuda()
        ae_optimizer = optimizer(sub_autoencoder)
        ae_scheduler = scheduler(
            ae_optimizer) if scheduler is not None else scheduler
        train(
            current_dataset,
            sub_autoencoder,
            epochs,
...
def predict(
    ...
    data_iterator = tqdm(dataloader, leave=False,
                         unit="batch", disable=silent,)
    features = []
