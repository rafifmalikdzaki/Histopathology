...
def create_dataset(subset: str, image_path: Optional[str] = None) -> Tuple[List[str], torch.Tensor]:
    ...
    X = df['Image'].apply(lambda x: os.path.join(image_path, "HeparUnifiedPNG", subset, x)).astype(str).to_list()
...
class ImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Ensure image has the correct shape for batching (C, H, W)
        if image.dim() == 2:
            # If grayscale image, add channel dimension
